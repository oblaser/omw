 ...
 *
 * A color object is considered valid when any value was set. Either with one of the `set()` or `set..()` functions or with one of the constructors. However the
...
//!
void omw::Color::opaque() { aValue = 0xFF; }

//!
//! Sets the alpha value to 0. Does not change the validity.
//!
void omw::Color::transparent() { aValue = 0; }

//! @param rgb Integer representation of the color
//!
//! Format: `0x00RRGGBB`
//!
void omw::Color::set(int32_t rgb) { setARGB(0xFF000000 | rgb); }
...
void omw::Color::set(const omw::Color& color) { setARGB(color.toARGB()); }
...
void omw::Color::set(const std::string& css)
{
    ...
    else if (colStr.length() == colStrLen) { col = omw::hexstoi(colStr); }
...
//! Format: `0xAARRGGBB`
//!
void omw::Color::setARGB(int32_t argb) { set((argb >> 16) & 0xFF, (argb >> 8) & 0xFF, argb & 0xFF, (argb >> 24) & 0xFF); }
...
uint8_t omw::Color::r() const { return rValue; }

//! @return The green value of the color
uint8_t omw::Color::g() const { return gValue; }

//! @return The blue value of the color
uint8_t omw::Color::b() const { return bValue; }

//! @return The alpha value of the color
uint8_t omw::Color::a() const { return aValue; }

//! @return An integer RGB representation of the color
//!
//! Format: `0x00RRGGBB`
//!
int32_t omw::Color::toRGB() const { return (toARGB() & 0x00FFFFFF); }
...
void omw::Color::from_wxW_RGB(uint32_t wxWCol) { from_wxW_RGBA(0xFF000000 | wxWCol); }
...
//! See also to_wxW_RGB().
//!
uint32_t omw::Color::to_wxW() const { return to_wxW_RGB(); }

//!
//! Format: `0xBBGGRR`
//!
uint32_t omw::Color::to_wxW_RGB() const { return (to_wxW_RGBA() & 0x00FFFFFF); }
...
//!
bool omw::operator==(const omw::Color& lhs, const omw::Color& rhs) { return ((lhs.isValid() == rhs.isValid()) && (lhs.toARGB() == rhs.toARGB())); }

//!
//! See `omw::operator==(const omw::Color&, const omw::Color&)`.
//!
bool omw::operator!=(const omw::Color& lhs, const omw::Color& rhs) { return !(lhs == rhs); }
...
int32_t omw::alphaComposit(int32_t a_ACCC, int32_t b_ACCC)
{
    ...
    const uint8_t tmpB[] = {
        (uint8_t)((b_ACCC >> 24) & 0xFF), // alpha
        (uint8_t)((b_ACCC >> 16) & 0xFF), // color component 1
        (uint8_t)((b_ACCC >> 8) & 0xFF),  // color component 2
        (uint8_t)(b_ACCC & 0xFF)          // color component 3
    };
    ...
    {
        const float aa = (float)tmpA[0] / 255.0f;
        const float ab = (float)tmpB[0] / 255.0f;
        const float ar = aa + ab * ((float)1 - aa);

        const float ca[3] = { (float)tmpA[1] / 255.0f, (float)tmpA[2] / 255.0f, (float)tmpA[3] / 255.0f };
        const float cb[3] = { (float)tmpB[1] / 255.0f, (float)tmpB[2] / 255.0f, (float)tmpB[3] / 255.0f };
        float cr[3];

        for (size_t i = 0; i < 3; ++i) cr[i] = ((ca[i] * aa) + (cb[i] * ab * ((float)1 - aa))) / ar;

        result = (int32_t)std::lround(ar * 255);
        result <<= 8;
        result |= (int32_t)std::lround(cr[0] * 255);
        result <<= 8;
        result |= (int32_t)std::lround(cr[1] * 255);
        result <<= 8;
        result |= (int32_t)std::lround(cr[2] * 255);
    }
    else result = b_ACCC;
...
//!
omw::Color omw::alphaComposit(const omw::Color& a, const omw::Color& b) { return b + a; }
...
//!
void omw::alphaComposit_apply(int32_t a_ACCC, int32_t& b_ACCC) { b_ACCC = omw::alphaComposit(a_ACCC, b_ACCC); }
...
//! If one of the colors is invalid, the result is also invalid. However all color values are computed anyway.
//!
void omw::alphaComposit_apply(const omw::Color& a, omw::Color& b) { b += a; }
